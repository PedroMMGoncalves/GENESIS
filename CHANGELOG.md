...
- **Tool 07 provenance sidecar** renamed from `_temporal_provenance.csv` to `_provenance.csv` — the `_temporal_` infix duplicated context already encoded in the parent `temporal/` subfolder, and the new name matches the mosaic tools' `{output}_provenance.csv` convention.
