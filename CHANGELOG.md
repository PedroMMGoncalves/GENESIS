...
- **Tool 07 provenance sidecar** renamed from `_temporal_provenance.csv` to `_provenance.csv` — the `_temporal_` infix duplicated context already encoded in the parent `temporal/` subfolder, and the new name matches the mosaic tools' `{output}_provenance.csv` convention. **Compat shim** writes both filenames this release so scripts reading the old name keep working; the legacy filename will be **dropped in v1.2**.
- **Mosaic output filenames now carry the compositor algorithm as a suffix.** Earlier versions hardcoded `_Geomedian` on Landsat regardless of compositor and produced compositor-anonymous filenames on S2 / ASTER (the compositor was only recorded in the provenance CSV). The new convention is `{mosaic_name}_{compositor_tag}` where `compositor_tag` is one of `Geomedian` (default, matches Landsat 1.0), `PerBandMedian`, or `PerBandP{N}` for percentile composites (e.g., `PerBandP25` — matches the user's existing manual naming style). ASTER's two products keep their `_Vnir` / `_VnirSwir` axis between the user-supplied name and the compositor tag (e.g., `Aster_V18_VnirSwir_PerBandP25`). The redundant `_Masked` suffix that S2 and Landsat used to append when an AOI was applied is **gone** — the masked result is renamed into the canonical compositor-tagged name, so the GDB Catalog shows one entry per run instead of two.
